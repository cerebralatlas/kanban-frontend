...
## 🔐 Phase 2: 认证系统 (优先级: 🔥 HIGH)

### 🌐 API 客户端设置

- [ ] **现代化HTTP客户端配置 (`src/api/client.ts`)**
  - [ ] 创建基于Fetch API的客户端
  - [ ] 配置请求/响应处理
  - [ ] 实现 JWT token 自动添加
  - [ ] 实现错误处理和重试逻辑

- [ ] **Vue Query 配置 (`src/api/query-client.ts`)**
  - [ ] 配置QueryClient实例
  - [ ] 设置缓存策略
  - [ ] 配置重试逻辑
...
- [ ] **认证 API (`src/api/auth.ts`)**
  ...
  - [ ] `login(data)` - 用户登录  
  - [ ] `getProfile()` - 获取用户信息
  - [ ] `refreshToken()` - 刷新token (如果需要)

### 🏪 简化的状态管理 (Pinia + Vue Query)

- [ ] **认证 Store (`src/stores/auth.ts`) - 简化版**
  - [ ] 用户状态管理 (user, token, isLoggedIn)
  - [ ] 登录/登出操作
  - [ ] token 持久化存储

- [ ] **UI状态 Store (`src/stores/ui.ts`)**
  - [ ] 主题状态管理
  - [ ] 侧边栏状态
  - [ ] 通知系统

- [ ] **认证 Composables (`src/composables/useAuth.ts`)**
  - [ ] 使用Vue Query管理认证相关数据获取
  - [ ] 登录/注册mutation
  - [ ] 用户信息query

### 🧩 认证组件

- [ ] **登录表单 (`src/components/auth/LoginForm.vue`) - 使用Naive UI**
  - [ ] 邮箱/用户名 + 密码表单
  - [ ] Naive UI表单验证
  - [ ] 加载状态和错误处理
  - [ ] "记住我" 功能

- [ ] **注册表单 (`src/components/auth/RegisterForm.vue`) - 使用Naive UI**
  - [ ] 完整注册表单 (邮箱、用户名、密码、确认密码)
  - [ ] 实时验证反馈
  - [ ] 密码强度指示器
